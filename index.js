...
function button() {
    ...
    sound(buttonTriggered);
    console.log(buttonTriggered);
    Animation(buttonTriggered)
}
}
...
document.addEventListener("keypress",function(event){
  sound(event.key);
  Animation(event.key);
  // event.key (event is argument and key fetch the letter that is pressed)
});
...
function Animation(button){
  ...
  setTimeout(function(){
    buttonPressed.classList.remove("pressed",1);
  })
}
